<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold">Editar Serie de Facturacion</h1>
            <p class="text-base-content/60 text-sm mt-1">{{ $invoiceSeries->prefix }} - {{ $invoiceSeries->fiscal_year }}</p>
        </div>
    ...
    <div class="mb-6">
        @switch($invoiceSeries->serie->value)
            ...
            @case(3)
                ...
                @break
        @endswitch
        <span class="badge badge-outline">{{ $invoiceSeries->fiscal_year }}</span>
    </div>
    ...
    <form wire:submit="update" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <fieldset class="fieldset">
                    ...
                    <input
                        ...
                        disabled
                    />
                    <p class="text-base-content/60 text-xs mt-1">El tipo no se puede cambiar</p>
                </fieldset>
                ...
                <fieldset class="fieldset">
                    ...
                    <input
                        type="text"
                        value="{{ $invoiceSeries->fiscal_year }}"
                        class="input w-full"
                        readonly
                        disabled
                    />
                    <p class="text-base-content/60 text-xs mt-1">El ano fiscal no se puede cambiar</p>
                </fieldset>
        ...
        <x-ui.card>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Current Numbers (read-only) -->
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">Numeracion actual</legend>
                    <div class="bg-base-200 rounded-lg p-3">
                        <div class="flex justify-between text-sm">
                            <span class="text-base-content/60">Numero inicial:</span>
                            <span class="font-mono">{{ $invoiceSeries->start_number }}</span>
                        </div>
                        <div class="flex justify-between text-sm mt-1">
                            <span class="text-base-content/60">Ultimo numero:</span>
                            <span class="font-mono font-bold">{{ $invoiceSeries->last_number }}</span>
                        </div>
                        <div class="flex justify-between text-sm mt-1">
                            <span class="text-base-content/60">Siguiente:</span>
                            <span class="font-mono text-primary">{{ $invoiceSeries->getNextNumber() }}</span>
                        </div>
                    </div>
                    <p class="text-base-content/60 text-xs mt-1">La numeracion no se puede modificar manualmente</p>
                </fieldset>
        ...
        <x-ui.card>
            ...
            <div class="bg-base-200 rounded-lg p-4">
                <p class="text-base-content/60 text-sm mb-2">Proximo numero de factura:</p>
                <p class="font-mono text-lg">
                    @php
                        $preview = str_replace(
                            ['{{prefix}}', '{{year}}', '{{number}}'],
                            [$prefix ?: $invoiceSeries->prefix, $invoiceSeries->fiscal_year, str_pad((string)$invoiceSeries->getNextNumber(), 5, '0', STR_PAD_LEFT)],
                            $number_format
                        );
                    @endphp
                    {{ $preview }}
