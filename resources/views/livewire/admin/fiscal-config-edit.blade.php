<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold">Editar Configuracion Fiscal</h1>
            <p class="text-base-content/60 text-sm mt-1">{{ $fiscalConfig->business_name }} ({{ $fiscalConfig->tax_id }})</p>
        </div>
    ...
    <form wire:submit="update" class="space-y-6">
        ...
        <x-ui.card>
            ...
            <fieldset class="fieldset">
                <legend class="fieldset-legend">Notas adicionales</legend>
                <textarea
                    wire:model="notes"
                    class="textarea w-full @error('notes') textarea-error @enderror"
                    rows="3"
                    placeholder="Notas sobre esta configuracion..."
                ></textarea>
                @error('notes')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <x-ui.card>
            ...
            <dl class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                    <dt class="text-base-content/60">Vigente desde</dt>
                    <dd>{{ $fiscalConfig->valid_from->format('d/m/Y') }}</dd>
                </div>
                <div>
                    <dt class="text-base-content/60">Vigente hasta</dt>
                    <dd>
                ...
                <div>
                    <dt class="text-base-content/60">Rango</dt>
                    <dd>{{ $fiscalConfig->validity_range }}</dd>
                </div>
            </dl>
            <p class="text-base-content/60 text-xs mt-4">Para cambiar la fecha de vigencia, crea una nueva configuracion.</p>
        </x-ui.card>
